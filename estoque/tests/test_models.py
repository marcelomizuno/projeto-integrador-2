...
class ProdutoTest(TestCase):
    def setUp(self):
        ...
        self.produto = Produto.objects.create(nome='product_test', descricao='product_description_test', codigo_de_barras='abc123', 
                       categoria=self.categoria, quantidade_minima=1, quantidade_atual=5, is_active=True)
    ...
    def test_quantity_increase(self, quantity=1):
        product_quantity = self.produto.quantidade_atual
        self.produto.incrementar(quantity)
        self.assertEqual(self.produto.quantidade_atual, product_quantity + quantity)
...
class MovimentacaoTest(TestCase):
    def setUp(self):
        ...
        self.produto = Produto.objects.create(nome='test', categoria=self.categoria, descricao='product test', codigo_de_barras='abc123', quantidade_minima=0, quantidade_atual=10, is_active=True)
    ...
    def test_movimentacao_entrada(self):
        quantidade_produtos = self.produto.quantidade_atual
        movimentacao = self.create_movimentacao('entrada')
        self.assertEqual(quantidade_produtos + movimentacao.quantidade, self.produto.quantidade_atual)
